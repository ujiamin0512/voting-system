...
async function handleApi(req, res, url) {
  ...
  if (p === '/api/vote' && m === 'POST') {
    ...
    // Normally the cookie and its localStorage mirror hold the SAME id, so this must be
    // deduplicated: passing one id twice made the second voted_devices insert collide
    // with the first and the whole ballot was rolled back as a repeat vote.
    const ids = [...new Set([cookieId, deviceId].filter(Boolean))];
