...
const publicView = c => ({ id: c.id, name: c.name, photo: c.photo, game: c.game || '', link: c.link || '', preview: c.preview || '' });

// Link preview: the page's og:image / twitter:image URL, the picture chat apps show for a
// link. Only the URL is stored; browsers load the image straight from the site. Best
// effort — any failure just means the tile falls back to an icon.
async function fetchPreview(link) {
  ...
  try {
    const r = await fetch(link, {
      ...
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; VotingBot/1.0; +link preview)', Accept: 'text/html' }
    });
    if (!r.ok || !/text\/html/i.test(r.headers.get('content-type') || '')) return '';
    const html = (await r.text()).slice(0, 512 * 1024);
    const meta = /<meta\s+[^>]*?(?:property|name)\s*=\s*["'](?:og:image(?::secure_url)?|twitter:image(?::src)?)["'][^>]*>/gi;
    for (const tag of html.match(meta) || []) {
      const m = tag.match(/content\s*=\s*["']([^"']+)["']/i);
      if (!m) continue;
      try {
        const u = new URL(m[1].trim(), r.url || link);
        if (/^https?:$/.test(u.protocol)) return u.href.slice(0, 2000);
      } catch {}
    }
    return '';
  } catch { return ''; }
...
// Refresh the preview whenever the link changes (and there is no uploaded cover).
async function withPreview(patch, existing) {
  if (patch.link !== undefined && patch.link !== (existing && existing.link))
    patch.preview = await fetchPreview(patch.link);
  return patch;
}
...
async function handleApi(req, res, url) {
  ...
  if (p === '/api/results' && m === 'GET') {
    ...
    return send(res, 200, {
      ...
      candidates: candidates.map(c => {
        // photoLen lets the projector notice a changed cover without downloading every photo.
        const v = { id: c.id, name: c.name, votes: c.votes, game: c.game || '', link: c.link || '',
          preview: c.preview || '', photoLen: (c.photo || '').length };
  ...
  if (p === '/api/submit' && m === 'POST') {
    ...
    await withPreview(patch, existing);
    if (existing) {
  ...
  if (p === '/api/candidates' && m === 'POST') {
    ...
    if (error) return send(res, 400, { error });
    await withPreview(patch, null);
    const c = { id: crypto.randomUUID(), photo: '', game: '', link: '', preview: '', ...patch, votes: 0, device: null };
