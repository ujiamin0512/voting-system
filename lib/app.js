...
async function handleApi(req, res, url) {
  ...
  if (p === '/api/poll' && m === 'GET') {
    ...
    return send(res, 200,
      { settings, candidates: candidates.map(publicView), deviceId: did, alreadyVoted },
  ...
  if (p === '/api/admin/state' && m === 'GET') {
    ...
    return send(res, 200, { settings, candidates, ...stats, storage: store.kind, voteUrl: `${origin}/vote` });
  }
