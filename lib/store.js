...
function pgStore(url) {
  ...
  const init = () => (ready ||= (async () => {
    ...
    await q(`ALTER TABLE candidates ADD COLUMN IF NOT EXISTS preview TEXT NOT NULL DEFAULT ''`);
  ...
  const toCandidate = r => ({
    ...
    game: r.game, link: r.link, preview: r.preview, device: r.device
  });
  ...
  return {
    ...
    async addCandidate(c) {
      await init();
      await q(`INSERT INTO candidates (id, name, photo, votes, game, link, preview, device)
               VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [c.id, c.name, c.photo, c.votes, c.game || '', c.link || '', c.preview || '', c.device || null]);
      return c;
    },
    async findCandidateByDevice(ids) {
      await init();
    ...
    async updateCandidate(id, patch) {
      await init();
      const cols = { name: 'name', photo: 'photo', votes: 'votes', game: 'game', link: 'link', preview: 'preview' };
      const sets = [], vals = [];
      for (const [key, col] of Object.entries(cols)) {
        if (patch[key] !== undefined) { vals.push(patch[key]); sets.push(`${col} = $${vals.length}`); }
      }
