...
function fileStore() {
  ...
  return {
    ...
    async castBallot(choices, ids, lock) {
      ...
      if (lock) db.voted.push(...ids.filter(id => !db.voted.includes(id)));
      save();
    },
...
function pgStore(url) {
  ...
  const toCandidate = r => ({ id: r.id, name: r.name, photo: r.photo, votes: r.votes });
  ...
  return {
    ...
    async addCandidate(c) {
      await init();
      await q(`INSERT INTO candidates (id, name, photo, votes) VALUES ($1, $2, $3, $4)`,
        [c.id, c.name, c.photo, c.votes]);
      return c;
    },
    async updateCandidate(id, patch) {
      await init();
      const cols = { name: 'name', photo: 'photo', votes: 'votes' };
      const sets = [], vals = [];
      for (const [key, col] of Object.entries(cols)) {
        if (patch[key] !== undefined) { vals.push(patch[key]); sets.push(`${col} = $${vals.length}`); }
      }
    ...
    async castBallot(choices, ids, lock) {
      ...
      try {
        ...
        await client.query(`UPDATE candidates SET votes = votes + 1 WHERE id = ANY($1::text[])`, [choices]);
...
module.exports = url ? pgStore(url) : fileStore();
